...
function WeatherInformations5Days({ weather5Days }) {
    ...
    return (
        <div>
            <h2 className='title-days'>Proximos Dias</h2>
            <div className="container-days">
                {next5DaysForecast.map(forecast => (
                    <div className="card-days" key={forecast.dt}>
                        <p className='day'>{convertDate(forecast)}</p>
                        {/*<img src={`http://api.openweathermap.org/img/wn/${forecast.weather[0].icon}.png`} alt="temp" /> */}
                        <svg
                            className="weather"
                            version="1.1"
                            id="Layer_1"
                        ...
                        >
                            ...
                            ></image></svg>
                        <p className='description-day'>{forecast.weather[0].description}</p>
                        <div className="temp-min-max">
                            <p className="temp-min">Min<br />{Math.round(forecast.main.temp_min)}°C</p>
                            <p className="temp-max">Max<br />{Math.round(forecast.main.temp_max)}°C</p>
                        </div>
                    </div>
                ))}
            </div>
